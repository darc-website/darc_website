...
export default function MemoEditor() {
    ...
    const [category, setCategory] = useState("언론보도"); // Dropdown selection
        ...
        if (quillRef.current) {
            ...
            try {
                ...
                if (response.ok) {
                    setTitle("");
                    setDate("");
                    setCategory("언론보도"); // or set to empty string if you prefer
                    setContent(null);

                    setIsEditing(false); // Exit editing mode
                } else {
                    setError(data.error || "자료실 저장에 실패했습니다.");
                }
            } catch (err) {
