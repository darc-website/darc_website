...
export async function POST(req) {
    try {
        ...
        return NextResponse.json({ message: '공지사항이 성공적으로 저장되었습니다.' }, { status: 201 });
    } catch (error) {
        console.error('공지사항 저장 에러:', error);
        return NextResponse.json({ error: '공지사항을 저장하는데 실패했습니다.' }, { status: 500 });
    }
}
...
export async function DELETE(req) {
    try {
        ...
        return NextResponse.json({ error: '공지사항을 삭제하는데 실패했습니다.' }, { status: 500 });
    }
}